...
5. 去掉顶部"新增参保员工"/"编辑员工"标题文字。
6. 底部两个操作按钮（"保存/提交"主按钮 + "完成，返回列表"次按钮）从上下堆叠改成同一行左右排列，方便连续添加流程里反复操作。
...
删掉 `<view class="title">{{id?'编辑员工':'新增参保员工'}}</view>` 这一行（`subtitle` 那行已经在上一轮改动里做成 `wx:if="{{!locked}}"` 了，这次连同 `title` 一起去掉，标题栏不再占用空间，页面从锁定/摘要卡片或分段控件直接开始）。

### 7. 底部按钮改成一行

当前底部是"保存/提交"主按钮独占一行，下面再单独一行"完成，返回列表"次按钮（仅 `locked` 时出现）。改成：

```html
<view class="row" style="gap:16rpx;margin-top:28rpx">
  <button class="primary grow" loading="{{saving}}" disabled="{{saving}}" bindtap="save">{{id?'保存员工信息':(locked?'保存并继续添加下一人':'提交参保审核')}}</button>
  <button wx:if="{{locked}}" class="secondary grow" bindtap="finish">完成</button>
</view>
```

未锁定时（`!locked`）只有主按钮，`.grow` 让它占满整行，行为和现在一样；锁定后两个按钮各占一半宽度并排，方便连续添加流程里反复点"保存并继续添加下一人"而不用视线上下移动。"完成，返回列表"文案缩短成"完成"以适配半宽按钮。

## 测试

无自动化测试基础设施，验证方式同前几次：`node -c employee-edit.js` 语法检查 + WeChat DevTools 手工验证清单：
- 新增模式（无 `id`）：无标题文字，顶部无岗位摘要卡（除非 locked），分段控件三选一切换正常，"更多选项"默认收起、点击展开生效/停保时间字段，底部只有一个占满整行的主按钮。
- 从首页岗位卡片进入（locked=true）：合并后的岗位摘要卡显示正确（企业/职业类别/岗位名/审核状态/已添加人数），无重复的橙色提示条；底部主按钮+"完成"按钮并排各占一半。
- 编辑模式（有 `id`）：无标题文字、无分段控件，"更多选项"默认展开，生效/停保时间字段可见可编辑，底部只有一个占满整行的主按钮（编辑模式下 `locked` 恒为 false，不会出现"完成"按钮）。
