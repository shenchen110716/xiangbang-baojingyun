...
public interface WorkPositionMapper {
    String COLS = "id, enterprise_id as enterpriseId, actual_employer_id as actualEmployerId, actual_employer as actualEmployer, " +
            "name, occupation_class as occupationClass, plan_id as planId, status, created_at as createdAt";
    ...
    @Insert("INSERT INTO work_positions (enterprise_id, actual_employer_id, actual_employer, name, occupation_class, plan_id, status, created_at) " +
            "VALUES (#{enterpriseId}, #{actualEmployerId}, #{actualEmployer}, #{name}, #{occupationClass}, #{planId}, #{status}, #{createdAt})")
