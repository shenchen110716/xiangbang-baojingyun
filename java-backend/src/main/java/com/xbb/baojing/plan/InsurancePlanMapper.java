...
public interface InsurancePlanMapper {
    String COLS = "id, insurer, insurer_id as insurerId, insurer_email as insurerEmail, name, coverage, occupation_classes as occupationClasses, " +
    ...
    @Select("<script>SELECT DISTINCT p." + "id, p.insurer, p.insurer_id as insurerId, p.insurer_email as insurerEmail, p.name, p.coverage, p.occupation_classes as occupationClasses, " +
    ...
    @Insert("INSERT INTO insurance_plans (insurer, insurer_id, insurer_email, name, coverage, occupation_classes, price, commission_rate, profit_amount, payment_mode, billing_mode, effective_mode, status, created_at) " +
            "VALUES (#{insurer}, #{insurerId}, #{insurerEmail}, #{name}, #{coverage}, #{occupationClasses}, #{price}, #{commissionRate}, #{profitAmount}, #{paymentMode}, #{billingMode}, #{effectiveMode}, #{status}, #{createdAt})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(InsurancePlan p);

    @Update("UPDATE insurance_plans SET insurer=#{insurer}, insurer_id=#{insurerId}, insurer_email=#{insurerEmail}, name=#{name}, coverage=#{coverage}, " +
