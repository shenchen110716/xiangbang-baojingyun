...
Page({
  ...
  onShow() { this.load(); },
  load() { return app.request('/positions', { silent: true }).then((items) => { const mapped = items.map((item) => ({ ...item, status_label: item.status === 'approved' ? '已定类' : item.status === 'rejected' ? '审核驳回' : item.status === 'supplement' ? '待补充材料' : item.video_count ? '待平台定类' : '待上传视频' })); this.setData({ items: mapped, loading: false }); this.filter(); }).catch(() => this.setData({ loading: false })); },
