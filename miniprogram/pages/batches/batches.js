...
Page({
  ...
  email(e) { const planId = Number(e.currentTarget.dataset.plan), kind = e.currentTarget.dataset.kind; wx.showModal({ title: kind === 'enrollment' ? '邮件发送新参名单' : '邮件发送停保名单', content: '系统将生成 CSV 附件并发送到该方案配置的保司邮箱。', success: (res) => { if (!res.confirm) return; app.request(`/enrollment/email?enterprise_id=${this.data.enterpriseId}&plan_id=${planId}&kind=${kind}&date=${this.data.date}`, { method: 'POST' }).then((data) => wx.showToast({ title: `已发送 ${data.people_count} 人`, icon: 'none' })); } }); },
