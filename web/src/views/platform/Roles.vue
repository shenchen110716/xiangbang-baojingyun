...
<template>
  <h1>角色管理</h1>
  <p class="sub">授权链的根。只有 PLATFORM_ADMIN 能改角色</p>
  ...
  <div class="card">
    ...
    <div class="row" style="gap:6px">
      ...
      <span v-for="r in mine" :key="r" class="tag ok">{{ r }}</span>
  ...
  <div class="card">
    ...
    <div class="row">
      ...
      <div class="field" style="flex:0 0 200px"><label>角色</label>
        <select v-model="role">
          <option value="PLATFORM_OPS">PLATFORM_OPS（运维）</option>
          <option value="PLATFORM_ADMIN">PLATFORM_ADMIN（管理员）</option>
        </select>
      </div>
  ...
  <div class="card" style="background:var(--surface-2)">
    ...
    <p class="hint" style="margin:0">
      授权链要有根：第一个 PLATFORM_ADMIN 不可能由别的管理员授予。
