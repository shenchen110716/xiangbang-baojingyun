...
class InvoiceIn(BaseModel): enterprise_id: int; account: Literal["premium","usage"] = "premium"; invoice_type: Literal["增值税普通发票","增值税专用发票"] = "增值税普通发票"; amount: float = Field(gt=0); title: str = Field(min_length=1,max_length=160); tax_no: str = ""; email: str = ""
